...
Func _Login($times = 0)
	While 1
		...
		$login = _HttpRequest(2, 'http://ftugate.ftu.edu.vn/default.aspx', $data_login, $cookie, 'http://ftugate.ftu.edu.vn/default.aspx', 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:78.0) Gecko/20100101 Firefox/78.0|Origin: http://ftugate.ftu.edu.vn|Origin: http://ftugate.ftu.edu.vn|DNT: 1|Connection: keep-alive|Upgrade-Insecure-Requests: 1')
		...
		Else
			...
			ElseIf (StringInStr($login, 'Server đang tải lại dữ liệu. Vui lòng trở lại sau 15 phút!') > 0) Then
				...
				Else
					...
					_Login($times + 1)
				EndIf
			ElseIf (StringInStr($login, "ctl00_Header1_Logout1_lblNguoiDung") > 0) Then
