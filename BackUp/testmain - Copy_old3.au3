...
$Account = GUICtrlCreateInput("1911110274", 82, 24, 155, 24, BitOR($GUI_SS_DEFAULT_INPUT, $WS_BORDER))
GUICtrlSetColor(-1, 0x00FF00)
...
$Password = GUICtrlCreateInput("3980", 322, 24, 152, 24, BitOR($GUI_SS_DEFAULT_INPUT, $ES_PASSWORD, $WS_BORDER))
GUICtrlSetColor(-1, 0x00FF00)
GUICtrlSetBkColor(-1, 0x000000)
...
While 1
	...
	Switch $nMsg
		...
		Case $List
			...
			ShellExecute(@ScriptDir & '\Show.html')
			Else
			$kqloc = _HttpRequest(4, 'http://ftugate.ftu.edu.vn/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx', '{"dkLoc":' & '"' & $sub_data[2] & '"' & '}', $cookie, 'http://ftugate.ftu.edu.vn/Default.aspx?page=dkmonhoc', 'X-AjaxPro-Method: LocTheoMonHoc')
			FileDelete(@ScriptDir & '\List.html')
			FileWrite(@ScriptDir & '\List.html', $kqloc[1])
...
Func _Reg()
	...
	If StringInStr($kqloc[1], 'Không có môn học được mở') > 0 Then
			_Text('Mã môn học hoặc mã lớp tín chỉ không chính xác.')
	ElseIf StringInStr(StringRegExp($kqloc[1], "align='center'>" & $sub_data[0] & "(.*?)\\r\\n", 1)[0], 'Hết') > 0 Then
	...
	Else
		...
		If $tach <> 0 Then
			...
			If $split_data[0] >= 13 Then
				...
				;		FileWrite(@ScriptDir&'\tienquyet.html',$reg[1])
				;		ShellExecute(@ScriptDir&'\tienquyet.html')
				;If StringInStr($reg[1], 'Ngoài thời gian đăng ký') <> 0 Then
					If $return[0] > 30 Then
						If Number($return[10]) == 0 Then
							If $return[8] == "" And $return[11] == "" Then
...
Func _Schedule()
	...
	$post_sch1 = _HttpRequest(2, 'http://ftugate.ftu.edu.vn/Default.aspx?page=thoikhoabieu', $data2send, $cookie, 'http://ftugate.ftu.edu.vn/Default.aspx?page=thoikhoabieu', 'Origin: http://ftugate.ftu.edu.vnUpgrade-Insecure-Requests: 1')
	$get_sch1 = _HttpRequest(2, 'http://ftugate.ftu.edu.vn/default.aspx?page=thoikhoabieu&sta=0', '', $cookie, 'http://ftugate.ftu.edu.vn/Default.aspx?page=thoikhoabieu', 'Upgrade-Insecure-Requests: 1')
