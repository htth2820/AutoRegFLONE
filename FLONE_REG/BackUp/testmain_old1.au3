...
$Form1 = GUICreate("AutoRegFTU by  Trần Thanh Hùng Version 0.1.0", 609, 412, 200, 135)
...
$Account = GUICtrlCreateInput("", 82, 24, 155, 24, BitOR($GUI_SS_DEFAULT_INPUT, $WS_BORDER))
GUICtrlSetColor(-1, 0x00FF00)
...
$Password = GUICtrlCreateInput("", 322, 24, 152, 24, BitOR($GUI_SS_DEFAULT_INPUT, $ES_PASSWORD, $WS_BORDER))
GUICtrlSetColor(-1, 0x00FF00)
GUICtrlSetBkColor(-1, 0x000000)
...
$CMD = GUICtrlCreateInput("MãMH:Mã lớp", 16, 368, 457, 24)
GUICtrlSetColor(-1, 0x00FF00)
GUICtrlSetBkColor(-1, 0x000000)
...
$Instruction = GUICtrlCreateButton("Instruction", 496, 70, 89, 25)
GUICtrlSetFont(-1, 10, 800, 0, "MS Sans Serif")
GUICtrlSetCursor(-1, 0)
GUICtrlSetColor(-1, 0x00FF00)
GUICtrlSetBkColor(-1, 0x000000)
...
While 1
	...
	Switch $nMsg
		...
		Case $Send
			_Reg()
		Case $Instruction
			MsgBox(64, 'Instruction', '[Insert Plan]:     Tạo bản kế hoạch đăng ký môn' & @CRLF & '[Reg Plan]:        Tự động đăng ký môn theo bản kế hoạch' & @CRLF & '[Clear All]:         Xóa bản kế hoạch và Kết quả đăng ký' & @CRLF &'[Set Alert]:         Đặt báo hiệu khi mở đăng ký' & @CRLF & '[Contact Me]:   Liên hệ tác giả khi cần giúp đỡ' & @CRLF & '[DELETE]:           Xóa môn học' & @CRLF & '[REG]:                Đăng ký môn học')
		Case $Delete
			_Del()
		Case $Contact
			_showAboutDialog('Auto Reg FTU', '1.1.0', 'Trần Thanh Hùng', Default, Default, 'https://www.facebook.com/darrenhtth')
		Case $Insert_plan
			MsgBox(64, 'Instruction', 'Nhập vào file Plan.txt theo cú pháp:' & @CRLF & 'Mã môn học:Mã lớp tín chỉ' & @CRLF & 'VD: XHH101E:XHH101E(1-1920).1_LT' & @CRLF & 'Mỗi môn ghi trên một dòng' & @CRLF & 'Ghi xong nhớ Save (Ctrl+S) rồi mới Close')
			FileWrite(@ScriptDir & '\Plan.txt', '')
			ShellExecute(@ScriptDir & '\Plan.txt')
		Case $Reg_plan
			$lines = _FileCountLines(@ScriptDir & '\Plan.txt')
			For $i = 1 To $lines
				GUICtrlSetData($CMD, '')
...
Func _Cookie()
	While 1
		...
		Global $source_login = _HttpRequest(4, 'http://ftugate.ftu.edu.vn/default.aspx?page=dangnhap')
		Global $cookie = _GetCookie($source_login[0])
		If @error Then
			_Text('Không lấy được Cookie, đang tiến hành thử lại....')
			ContinueLoop
		Else
			Sleep(1000)
...
Func _Login($times = 0)
	While 1
		...
		$data_login = _Data2SendEncode('__EVENTTARGET=&__EVENTARGUMENT=&__VIEWSTATE=' & $in__VIEWSTATE[0] & '&ctl00$ContentPlaceHolder1$ctl00$txtTaiKhoa=' & $acc & '&ctl00$ContentPlaceHolder1$ctl00$txtMatKhau=' & $pass & '&ctl00$ContentPlaceHolder1$ctl00$btnDangNhap=Đăng Nhập')
		$login = _HttpRequest(4, 'http://ftugate.ftu.edu.vn/default.aspx?page=dangnhap', $data_login, $cookie, '', 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:68.0) Gecko/20100101 Firefox/68.0|DNT: 1|Upgrade-Insecure-Requests: 1')
		If @error Then
			_Text('Không lấy được nội dung trang tín chỉ. Đang thử lại....')
		Else
			If (StringInStr($login[1], "ctl00_ContentPlaceHolder1_ctl00_lblError") > 0) Then
			...
			ElseIf (StringInStr($login[1], 'Server đang tải lại dữ liệu. Vui lòng trở lại sau 15 phút!') > 0) Then
				...
				Else
					...
					_Login($times + 1)
				EndIf
			ElseIf (StringInStr($login[1], "ctl00_Header1_ucLogout_lblNguoiDung") > 0) Then
...
;==============================_Logout==========================
Func _Logout()
	$source_logout = _HttpRequest(2, 'http://ftugate.ftu.edu.vn/', '', $cookie, '', 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:68.0) Gecko/20100101 Firefox/68.0|DNT: 1|Upgrade-Insecure-Requests: 1')
	...
	$logout = _HttpRequest(4, 'http://ftugate.ftu.edu.vn/', $data_logout, $cookie, '', 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:68.0) Gecko/20100101 Firefox/68.0|DNT: 1|Upgrade-Insecure-Requests: 1')
	If @error Then
...
Func _Load()
	...
	If StringInStr($load_header[0], 'HTTP/1.1 200 OK') > 0 Then
		...
		_Text('Nhập lệnh: "Mã môn học : Mã lớp tín chỉ" để tiến hành đăng kí (REG) hoặc xóa môn (DELETE)')
		_Text('VD: Nhập XHH101E:XHH101E(1-1920).1_LT')
	Else
...
Func _Reg()
	...
	$fix=$sub_data[1]&(StringReplace(StringReplace(StringReplace($sub_data[2], '(','\('),')','\)'),'/','\/'))
	$kqloc = _HttpRequest(4, 'http://ftugate.ftu.edu.vn/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx', '{"dkLoc":' & '"' & $sub_data[1] & '"' & '}', $cookie, 'http://ftugate.ftu.edu.vn/Default.aspx?page=dkmonhoc', 'X-AjaxPro-Method: LocTheoMonHoc|DNT: 1')
	...
	If $tach <> 0 Then
		...
		If $split_data[0] >= 13 Then
			...
			$reg = _HttpRequest(4, 'http://ftugate.ftu.edu.vn/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx', $data_reg, $cookie, 'http://ftugate.ftu.edu.vn/Default.aspx?page=dkmonhoc', 'X-AjaxPro-Method: DangKySelectedChange|DNT: 1')
...
;			If $reg[1] <> '{"value":""}' Then
			If StringInStr($reg[1],'56F15F12"  disabled')<>0 Then
				If $return[0] > 30 Then
...
Func _Del()
	...
	$del = _HttpRequest(4, 'http://ftugate.ftu.edu.vn/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx', $data_del, $cookie, 'http://ftugate.ftu.edu.vn/default.aspx?page=dkmonhoc', 'X-AjaxPro-Method: XoaKQDKTheoMaDK|DNT: 1')
...
Func _Alert()
	...
	While 1
		$dkmh=_HttpRequest(2,'http://ftugate.ftu.edu.vn/Default.aspx?page=dkmonhoc','',$cookie)
		If StringInStr($dkmh,'56F15F12"  disabled')>0 Then
			ContinueLoop
		Else
