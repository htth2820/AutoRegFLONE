...
While 1
	...
	Switch $nMsg
		...
		Case $Insert_plan
			MsgBox(64, 'Instruction', 'Nhập Mã lớp tín chỉ vào file Plan.txt' & @CRLF & 'VD: XHH101E(1-1920).1_LT' & @CRLF & 'Mỗi môn ghi trên một dòng' & @CRLF & 'Ghi xong nhớ Save (Ctrl+S) rồi mới Close')
...
Func _Reg()
	...
	$sub_data[0]=(StringReplace(StringReplace(StringReplace($sub_data[2], '(', '\('), ')', '\)'), '/', '\/'))
	...
	If $tach <> 0 Then
		...
		If $split_data[0] >= 13 Then
			...
			;		ShellExecute(@ScriptDir&'\after.html')
			;			If $reg[1] <> '{"value":""}' Then
			;If StringInStr($reg[1], '56F39F62"  disabled') <> 0 Then
				If $return[0] > 30 Then
	...
	#cs						$check_trung = _HttpRequest(4, 'http://ftugate.ftu.edu.vn/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx', '{}', $cookie, 'http://ftugate.ftu.edu.vn/default.aspx?page=dkmonhoc', 'X-AjaxPro-Method: KiemTraTrungNhom')
							$luu = _HttpRequest(4, 'http://ftugate.ftu.edu.vn/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx', '{}', $cookie, 'http://ftugate.ftu.edu.vn/default.aspx?page=dkmonhoc', 'X-AjaxPro-Method: LuuDanhSachDangKy')
		#ce					$luu_hople = _HttpRequest(4, 'http://ftugate.ftu.edu.vn/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx', '{"isCheckSongHanh":false,"ChiaHP":false}', $cookie, 'http://ftugate.ftu.edu.vn/default.aspx?page=dkmonhoc', 'X-AjaxPro-Method: LuuDanhSachDangKy_HopLe')
...
Func _Alert()
	...
	While 1
		$dkmh = _HttpRequest(2, 'http://ftugate.ftu.edu.vn/Default.aspx?page=dkmonhoc', '', $cookie)
		If StringInStr($dkmh, '56F39F62"  disabled') > 50 Then
			ContinueLoop
